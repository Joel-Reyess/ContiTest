...
const dir = resolve("build", "assets");

let archivos;
try {
  archivos = readdirSync(dir).filter((f) => f.endsWith(".js"));
} catch {
  console.error(`✖ No existe ${dir}. ¿Corrió vite build?`);
  process.exit(1);
}
...
if (!hallados.has(esperado) || hallados.has(prohibido)) {
  console.error(`✖ Build ${modo.toUpperCase()} INVÁLIDO: puertos de API en el bundle = ${lista}`);
...
console.log(`✔ Build ${modo.toUpperCase()} correcto: API en puerto ${esperado} (puertos hallados: ${lista}).`);

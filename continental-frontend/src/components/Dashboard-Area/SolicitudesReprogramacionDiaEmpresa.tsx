...
export function SolicitudesReprogramacionDiaEmpresa() {
    ...
    const cargar = async () => {
        setLoading(true)
        try {
            const data = await reprogramacionDiaEmpresaService.getSolicitudesArea()
            setSolicitudes(data)
        } catch {
            toast.error('Error al cargar solicitudes de reprogramación día empresa')
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleAprobar = async () => {
        ...
        setProcesando(true)
        try {
            await reprogramacionDiaEmpresaService.aprobarRechazar({
                solicitudId: paraAprobar.id,
                aprobada: true,
            })
            toast.success('Solicitud aprobada. Día empresa reprogramado y reflejado como "C".')
            setParaAprobar(null)
            await cargar()
        } catch (e: any) {
    ...
    return (
        <div className="space-y-4">
                ...
                {filtros.map(f => (
                    ...
                    >
                        ...
                        {f === 'Pendiente' && pendientes > 0 && (
                            <span className="ml-1 bg-red-500 text-white rounded-full px-1.5 py-0.5 text-[10px]">{pendientes}</span>
                        )}
                    </button>
                ))}
            </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-gray-200">
                                ...
                                <th className="text-left py-2 px-3 text-xs font-medium text-gray-500 uppercase">Justificación</th>
                            ...
                            {filtradas.map(s => (
                                <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50">
                                    <td className="py-2 px-3">
                                        ...
                                        {s.nomina && <p className="text-xs text-gray-500">Nómina: {s.nomina}</p>}
                                    </td>
                                    <td className="py-2 px-3 text-gray-700 text-sm">{s.nombreSolicitadoPor}</td>
                                    <td className="py-2 px-3">
                                        <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 text-xs font-medium">
                                            {MOTIVO_LABEL[s.motivoTipo as MotivoTipo] ?? s.motivoTipo}
                                        </span>
                                    </td>
                                    ...
                                    <td className="py-2 px-3 text-xs text-gray-700 max-w-xs">
                                        <p className="line-clamp-2">{s.justificacion || <span className="text-gray-400 italic">—</span>}</p>
                                        {s.motivoRechazo && (
            ...
            {paraAprobar && (
                <ApproveModal
                    show
                    onClose={() => setParaAprobar(null)}
                    onConfirm={handleAprobar}
                    solicitudId={String(paraAprobar.id)}
                    nombreEmpleado={paraAprobar.nombreEmpleado || ''}
                    tipoSolicitud="Reprogramación día empresa"
                    fechaActual={paraAprobar.fechaOriginal}
                    fechaReprogramacion={paraAprobar.fechaNueva}
                />
            )}
            {paraRechazar && (
                <RejectModal
                    ...
                    solicitudId={String(paraRechazar.id)}
                    nombreEmpleado={paraRechazar.nombreEmpleado || ''}
                />
            )}
        </div>
    )

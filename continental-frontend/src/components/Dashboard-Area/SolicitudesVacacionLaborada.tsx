...
export function SolicitudesVacacionLaborada() {
    ...
    const [solicitudParaAprobar, setSolicitudParaAprobar] = useState<VacacionLaborada | null>(null)
    const [solicitudParaRechazar, setSolicitudParaRechazar] = useState<VacacionLaborada | null>(null)

    const cargar = async () => {
        setLoading(true)
        try {
            const data = await vacacionLaboradaService.getSolicitudesArea()
            setSolicitudes(data)
        } catch {
            toast.error('Error al cargar solicitudes de vacación laborada')
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleAprobar = async () => {
        ...
        setProcesando(true)
        try {
            await vacacionLaboradaService.aprobarRechazar({
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    <h3 className="font-semibold text-gray-800">Vacaciones laboradas</h3>
                    {pendientes > 0 && (
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-gray-200">
                                ...
                                <th className="text-left py-2 px-3 text-xs font-medium text-gray-500 uppercase">Día trabajado</th>
                            ...
                            {solicitudesFiltradas.map(s => (
                                <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50">
                                    ...
                                    <td className="py-2 px-3 text-xs text-gray-700 max-w-xs">
                                        <p className="line-clamp-2">{s.motivo || <span className="italic text-gray-400">—</span>}</p>
                                        {s.motivoRechazo && (
            ...
            {solicitudParaAprobar && (
                <ApproveModal
                    ...
                    tipoSolicitud="Vacación laborada"

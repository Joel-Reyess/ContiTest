...
.DESCRIPTION
    Publica el backend en el puerto 5060 y el frontend en el 5175, contra la
...
param(
    ...
    [string]$RaizDestino = "C:\inetpub\vacaciones-test",
    [string]$SitioApi    = "Vacaciones-Test-Api",
    [switch]$SoloBackend,
    [switch]$SoloFrontend,
...
$rutaApi  = Join-Path $RaizDestino "api"
$rutaWeb  = Join-Path $RaizDestino "web"
...
if (-not $SoloBackend) {
    ...
    Push-Location $front
    try {
        # build:test usa .env.test, que apunta al backend 5060.
        npm run build:test
...
Write-Host "  Front: http://slas052a:5175"
Write-Host "  API:   http://slas052a:5060/swagger"
Write-Host ""
